...
function maintain_panel_structure(){
	...
	//hero import processor
	jQuery('.hero_processing_import').css({
		'min-height': (window_height) - parseInt(jQuery('.hero_admin').css('border-top')) +'px',
		'top': parseInt(jQuery('.hero_admin').css('border-top')) +'px'
	});
}
...
function load_json_menu_object(){
	jQuery.getJSON(plugin_url +'objects/menu_object.js', function(data){
...
function load_sub_view(id, viewpath, view){
	if(view != cur_sub_view){
		...
		jQuery('.hero_viewport').fadeOut(100, function(){
			...
			//load core
			jQuery('.hero_viewport').load(core_view_path +'views/'+ viewpath + view +'.view.php?vp='+ core_view_path +'views/'+ viewpath, function(){
				//map data object
				trigger_hplugin_event('map-data-object');
...
function launch_hero_popup(path_to_html,load_method,update_method,cancel_method,json){
	...
	//load content
	jQuery('.hero_popup_inner').load(core_view_path +'views/'+ path_to_html, function(){
