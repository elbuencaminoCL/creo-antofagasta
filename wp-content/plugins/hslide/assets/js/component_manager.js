...
function bind_field_convert(){
    jQuery('.hero_main, .hero_popup_main, .hero_popup_main, .hero_custom_popup_holder').find('input').each(function(index, element){
        switch(element.type){
			case 'text':
				...
				if(jQuery(this).data('hero_type') == 'int'){ jQuery(this).addClass('hero_int_only'); };
				if(jQuery(this).data('hero_type') == 'dec'){ jQuery(this).addClass('hero_dec_only'); };
	...
	jQuery('.hero_dec_only').on('keydown keyup paste', function(event){
		hero_force_dec(this);
	});
    jQuery('.hero_main, .hero_popup_main, .hero_popup_main, .hero_custom_popup_holder').find('textarea').each(function(index, element) {
		if(jQuery(this).data('bound') != 'set'){ bind_check(element); };
	});
    jQuery('.hero_main, .hero_popup_main, .hero_popup_main, .hero_custom_popup_holder').find('select').each(function(index, element) {
	...
	jQuery('.color_picker').each(function(index, element) {
		if(jQuery(this).data('bound') != 'set'){
			...
			//generate guid
			var component_guid = hplugin_guid();
			bind_color_html(element, component_guid, current_value, picker_id, picker_name, picker_class);	
		}
	});
	...
	jQuery('.hero_slider').each(function(index, element) {
		if(jQuery(this).data('bound') != 'set'){
			if(jQuery(this).data('bound') != 'set'){ bind_slider(element); }
		}
	});
...
function hide_hplugin_tooltip(){
	jQuery('.hplugin-tooltip-container').remove();
}
...
function bind_input_change_listener(the_field, the_current_color){
	jQuery(the_field).on('change', function(){
...
//create sliders
function bind_slider(element){
	var bind_link = jQuery(element).data('bind_link');
	var cur_value = jQuery('#'+ bind_link).val();
	var min_val = jQuery(element).data('min');
	var max_val = jQuery(element).data('max');
	var step_val = jQuery(element).data('step');
	jQuery('#'+ bind_link).attr('readonly', true);
	jQuery(element).slider({
		...
		slide: function(event, ui){
			jQuery('#'+ bind_link).val(ui.value).trigger('change');
		}
    });
}
...
function populate_select(element){
	...
	jQuery('#'+element.id + ' option').each(function(index, option) {      
		drop_down_row += '<div class="hero_drop_row" data-value="'+jQuery(this).val()+'">'+ toTitleCase(option.text) +'</div>';	
		if(jQuery(this).is(':selected')){
			jQuery('.'+element.id+ ' .hero_selected span').html(toTitleCase(option.text));
		}	
    });
...
//force decimal
function hero_force_dec(object){
	var val = jQuery(object).val();
	if(!/^\d+(\.\d{0,2})?$/.test(val)){
		jQuery(object).val(jQuery(object).val().replace(/[^0-9\\.]+/g, ''));
	}
